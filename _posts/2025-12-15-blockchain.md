...
比特币的发行机制：
- 总量恒定，永不增发：只发行共计约（略少于）2100万枚。
- 挖矿：矿工生成记录交易的区块的机制，被自动控制至约10分钟产出一次，每次产出会给与矿工$\overset{coinbase}{铸币}$奖励作为发行。铸币奖励最初为50枚，每产出21万个区块减半，约四年进行一次。
...
- 1976 迪菲、赫尔曼、默克尔团队 非对称加密思想
- 1977 RSA算法
- 1982 大卫·乔姆 盲签技术
- 1990 DigiCash公司 eCash
- 1991 齐默曼 PGP
- 1993 工作量证明思想
- 1994 智能合约
- 1998 BitGold
- 1999 工作量证明机制
- 2002 HashCash 工作量证明机制的第一个大规模运用

比特币是这些项目的集大成者：
### 哈希/散列（函数/算法）
定义：将大型数据转换为固定长度的值的函数，要求正向易捷、逆向难杂、异入异出、输入敏感。
...
非对称加密的理论来源：
- 大整数分解问题，如RSA算法：根据数论，寻求两个大素数比较简单，而将它们的乘积进行因式分解却极其困难，因此可以将乘积公开作为加密密钥。
- 离散对数问题，如DSA算法
- 椭圆曲线，如ECC算法。比特币中采用了SECP256K1

### 数字证书
包含拥有者、公钥、有效期、发证机构、签名等信息，用于证明拥有者的身份。
...
- 除此之外还有4B大小魔数（比特币中为0xD9B4BEF9，比特币测试网中为0xD709110B）、4B大小区块大小、交易数量等信息。
...
>
> 已知结果：当将军之间传达的是口头消息时，要求叛变的将军比例小于$\frac{1}{3}$。
